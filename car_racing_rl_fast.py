...
class Track:
    def __init__(self):
        ...
        pygame.draw.ellipse(self.track_surface, GREEN, [50, 50, 700, 500])
        
        # Draw inner boundary (grass hole)
        pygame.draw.ellipse(self.track_surface, BLACK, [150, 150, 500, 300])
...
def train(headless=False, speed_multiplier=1, max_episodes=1000):
    if not headless:
    ...
    while running and episode < max_episodes:
        ...
        while not episode_done and step < max_steps:
            if not headless:
                # Limit FPS based on speed multiplier
            ...
            # Render (only if not headless and not too fast)
            if not headless and speed_multiplier <= 10:
                track.draw(screen)
                car.draw(screen)
        ...
        if episode % 50 == 0:
            ...
            print(f"Episode {episode}/{max_episodes} - Model saved - Time: {elapsed:.1f}s")
        
        if episode % 10 == 0:
            avg_reward = np.mean(episode_rewards[-10:]) if len(episode_rewards) >= 10 else np.mean(episode_rewards)
            print(f"Episode {episode}, Avg Reward (last 10): {avg_reward:.2f}, Epsilon: {agent.epsilon:.3f}, Best Distance: {best_distance:.1f}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='2D Car Racing with Reinforcement Learning')
    parser.add_argument('--headless', action='store_true', help='Train without rendering (much faster)')
    parser.add_argument('--speed', type=int, default=1, help='Speed multiplier (1-100, default: 1)')
    ...
    train(headless=args.headless, speed_multiplier=args.speed, max_episodes=args.episodes)
